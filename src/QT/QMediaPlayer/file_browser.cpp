...
file_browser::file_browser(QObject* parent) : QObject(parent) 
{
    ...
    file_list = new QList<browser_item*>;
    file_list->clear();
 ...
 */
QString file_browser::get_file_name_by_index (uint32_t index)
{
uint16_t item_cnt = 0;
...
//    if(index<0) return NULL;
    ...
    item_cnt = file_list->size();

    if(item_cnt>0)
    {
        if((dir_counter+index)<file_list->size())
        {
                return file_list->at((dir_counter+index))->item_name;
        }
    }
...
uint16_t item_cnt = 0;
    ...
    if(index>item_counter || index<0) return NULL;

    item_cnt = file_list->size();
    
    if(item_cnt>0 && index<file_list->size()) 
    { 
        return file_list->at(index);   
    }
 ...
 * @return NULL jesli blad
 */
QString file_browser::get_dir (uint32_t index)
{
uint16_t item_cnt = 0;
    ...
    if(dir_counter>0 && (index+1)<file_list->size()) return file_list->at(index+1)->item_name;
...
bool file_browser::add_supported_extension(const char *extension)
{
  ...
  if(supported_formats_cnt==0)
    {
      supported_formats_tab = new QString[1];
  ...
  else
    {
      if(supported_formats_tab!=NULL)
        {
          ...
          tmp = new QString[supported_formats_cnt];
      ...
      supported_formats_cnt++;
      supported_formats_tab = new QString[supported_formats_cnt];
 ...
 */
int file_browser::refresh_file_list (QString P)
{
    QString s1 = "",s2 = "",filename,temp;
    ...
    if(access(path.toAscii().data(),F_OK)!=0){

        empty_dir_flag = true;
        return -1;
    }
   
//    printf("start path:%s\n",start_path.toAscii().data());
    temp = path;
    
    numberOfFiles = scandir(path.toAscii().data(), &nameList, 0, 0);
  ...
  if(numberOfFiles==0 || numberOfFiles==-1)
	...
	numberOfFiles = 1;
        ...
        if(item!=NULL)
        {
            item->item_name.clear();
            item->item_type = 2;
            item->pos_on_list = item_counter;
//            printf("%s %d %d %d %d \n",__FUNCTION__,__LINE__,current_source,get_sd1_status(),get_usb1_status());
            if(current_source==SOURCE_SD1 && get_sd1_status()==false)item->item_name="NO SD";
            else
            {
                if(current_source==SOURCE_USB1 && get_usb1_status()==false)item->item_name="NO USB";
            }
...
//      printf("%s  %s\n",current_path.toAscii().data(),start_path.toAscii().data());
...
//==============================================================
  ...
  for (int i = 0; i < numberOfFiles; i++)
      ...
      if(nameList[i]->d_type== DT_REG/*0x08*/)
	{
	    ...
	    int res = stat(temp.toAscii().data(),&file_info);
...
bool file_browser::check_supported_extension(char *file_name)
{
  ...
  if(pch!=NULL)
  {
    for(unsigned int i = 0;i<supported_formats_cnt;i++)
      {
        if(!strcmp((pch),supported_formats_tab[i].toAscii().data())) return true;
      }
  }
...
void file_browser::set_base_path (QString path)
{
    base_path = path;
...
char *str = NULL;
     ...
     if(idx<file_list->size()==false) return false;
...
//	    cpy.erase ((cpy.end ()-1));
	    ...
	    current_path = str;
            ...
            current_dir.clear();
            current_dir.append(strrchr(current_path.toAscii().data(),'/'));
            current_dir.remove("/");
        ...
        {
            current_dir.clear();
            current_dir.append(it->item_name);
            current_path.append (it->item_name);
            current_path.append ("/");
            base_path = current_path;
            refresh_file_list (NULL);
...
void file_browser::execute_user_cb_on_file_select(QString file_name)
{
//  FILE * fp = NULL;
