...
volatile int src_ev_cnt = 0;
volatile int gst_queue_src_activate_push_cnt = 0;
...
volatile int gloopst_queue_loop_cnt = 0;
GST_BOILERPLATE_FULL (GstQueue3, gst_queue, GstElement,GST_TYPE_ELEMENT, _do_init);
...
static GstFlowReturn gst_queue_push_one (GstQueue3 * queue);
...
static void gst_queue_locked_flush (GstQueue3 * queue);
...
static gboolean gst_queue_is_empty (GstQueue3 * queue);
static gboolean gst_queue_is_filled (GstQueue3 * queue);
...
static void
gst_queue_class_init (GstQueue3Class * klass)
{
  ...
  gst_queue_signals[SIGNAL_UNDERRUN] =
      ...
      G_STRUCT_OFFSET (GstQueue3Class, underrun), NULL, NULL,
  ...
  gst_queue_signals[SIGNAL_RUNNING] =
      ...
      G_STRUCT_OFFSET (GstQueue3Class, running), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  /**
   * GstQueue::overrun:
   * @queue: the queue instance
  ...
  gst_queue_signals[SIGNAL_OVERRUN] =
      ...
      G_STRUCT_OFFSET (GstQueue3Class, overrun), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
  /**
   * GstQueue::pushing:
   * @queue: the queue instance
  ...
  gst_queue_signals[SIGNAL_PUSHING] =
      ...
      G_STRUCT_OFFSET (GstQueue3Class, pushing), NULL, NULL,
      g_cclosure_marshal_VOID__VOID, G_TYPE_NONE, 0);
...
static void
gst_queue_init (GstQueue3 * queue, GstQueue3Class * g_class)
{
...
{
  GstQueue3 *queue = GST_QUEUE (object);
 ...
 // printf("###### src_ev_cnt:%d gloopst_queue_loop_cnt:%d gst_queue_src_activate_push:%d\n",src_ev_cnt,gloopst_queue_loop_cnt,gst_queue_src_activate_push_cnt);
...
*/
  ...
  if (GST_PAD_LINK_SUCCESSFUL (result)) {
    ...
    //  printf("gst_queue_src_activate_push:uruchom gst_queue_loop 2\n");
...
static void
update_time_level (GstQueue3 * queue)
{
 ...
 * level of queue. */
static void
apply_segment (GstQueue3 * queue, GstEvent * event, GstSegment * segment,
    gboolean sink)
{
  gboolean update;
  GstFormat format;
...
static void
apply_buffer (GstQueue3 * queue, GstBuffer * buffer, GstSegment * segment,
...
static void
gst_queue_locked_flush (GstQueue3 * queue)
{
  while (!g_queue_is_empty (queue->queue)) {
    GstMiniObject *data = g_queue_pop_head (queue->queue);
...
static inline void
gst_queue_locked_enqueue_buffer (GstQueue3 * queue, gpointer item)
{
...
static inline void
gst_queue_locked_enqueue_event (GstQueue3 * queue, gpointer item)
{
...
gst_queue_locked_dequeue (GstQueue3 * queue, gboolean * is_buffer)
{
  GstMiniObject *item;
 ...
 // printf("#### %s %d \n",__FUNCTION__,__LINE__);
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_FLUSH_STOP:
    {
     ...
     //     printf("gst_queue_src_activate_push:uruchom gst_queue_loop 4\n");
...
static gboolean
gst_queue_is_empty (GstQueue3 * queue)
{
  if (queue->queue->length == 0)
    return TRUE;
...
static gboolean
gst_queue_is_filled (GstQueue3 * queue)
{
...
static void
gst_queue_leak_downstream (GstQueue3 * queue)
{
...
gst_queue_chain (GstPad * pad, GstBuffer * buffer)
{
  ...
  queue = (GstQueue3 *) GST_OBJECT_PARENT (pad);
...
static void
gst_queue_push_newsegment (GstQueue3 * queue)
{
  GstSegment *s;
  GstEvent *event;
...
static GstFlowReturn
gst_queue_push_one (GstQueue3 * queue)
{
...
gst_queue_loop (GstPad * pad)
{
  ...
  queue = (GstQueue3 *) GST_PAD_PARENT (pad);
  
  gloopst_queue_loop_cnt++;
...
out_flushing:
  {
    ...
    GstFlowReturn ret = queue->srcresult;
    gboolean res;
    res = gst_pad_pause_task (queue->srcpad);
...
{
  gboolean res = TRUE;
  GstQueue3 *queue = GST_QUEUE (gst_pad_get_parent (pad));
...
gst_queue_src_activate_push (GstPad * pad, gboolean active)
{
  ...
  gst_queue_src_activate_push_cnt++;
...
static void
queue_capacity_change (GstQueue3 * queue)
{
...
static struct _elements_entry _elements[] = {
  ...
  {"queue_MM", GST_RANK_NONE, gst_queue_get_type},
  {NULL, 0},
};

#ifndef PACKAGE
#define PACKAGE "mySECONDplugin"
#endif

static gboolean plugin_init2 (GstPlugin * plugin)
{
